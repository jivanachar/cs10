...
public class ImageProcessingGUI0 extends DrawingGUI {
	...
	public void handleKeyPress(char op) {
		...
		else if (op=='d') { // brush down
			brush = true;
		}
		else if (op=='u') { // brush up
			brush = false;
		}
		else if (op=='+') { // increase radius
			radius += 1;
		}
		else if (op=='-') { // decrease radius
			if (radius > 0) {
				radius -= 1;
			}
		}
		else if (op=='r') { // set color to red
			color = Color.red;
		}
		else if (op=='g') { // set color to green
			color = Color.green;
		}
		else if (op=='b') { // set color to blue
			color = Color.blue;
		}
		else {
